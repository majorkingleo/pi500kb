...
void print_usage(const char *prog) {
    std::cout << std::format(
        ...
        "  --config <path>         Load keyboard layout JSON config\n"
        "                          e.g. --config config/keymap_de.json\n"
        "  --test-italian          Demo: built-in Italian layout\n"
        "  --host-os <os>          windows (Alt+numpad) | linux (WIP)\n"
        "\n"
        "Other options:\n"
...
int main(int argc, char *argv[]) {
    ...
    /*
     * AI generated by GitHub Copilot - DeepSeek V4 Flash
     * Load keyboard layout from JSON config.
     * Default: no keymap, just raw HID forwarding.
     * --test-italian: load built-in Italian demo.
     * --config <path>: load custom JSON config.
     */
    if (!config_path.empty()) {
        if (!load_keymap_from_json(config_path)) {
            std::cerr << "Config failed, continuing with raw forwarding" << std::endl;
        }
    } else if (test_italian) {
        load_keymap_italian();
    } else {
        std::cout << "No config loaded — raw forwarding only." << std::endl;
        std::cout << "  Try: --test-italian (demo) or --config config/keymap_it.json" << std::endl;
    }
    /* End of AI generated block */
